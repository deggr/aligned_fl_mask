...
def align_models(anchor_index, weights_results, masks_results, permutation_spec):
    ...
    target_model_ids = np.delete(np.arange(args.clients), anchor_index)
...
for server_round in tqdm(range(args.rounds)):
    ...
    # for all clients. we will proceed by aligning the weights to an anchor model, before applying the mask and adding
    # the masked received weights to the aggregate, and adding the mask
    # to the aggregate as well.
    
    # 1. Permute the client weights and masks wrt. to an anchor model
    if ANCHOR_SELECTION_RULE == 'random':
        anchor_index = np.random.choice(a=np.arange(10))
